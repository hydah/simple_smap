...
struct UserKey {
    ...
    std::string ToString() {
        return std::to_string(sdkappid_) + "-" + std::to_string(roomid_) + "-" + std::to_string(tinyid_);
    }
};
...
int main() {
    ssmap::simple_smap<User, UserKey, UserValidate, UserGetKey, ImplementWithKey, UserKeyHashFunc> smap;
